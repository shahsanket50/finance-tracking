...
function CCDrillDown({ leg }: { leg: ResolverPairing["legs"][number] }) {
  ...
  function toggle() {
    if (!open && txns === null) {
      ...
      fetch(
        `/api/v1/accounts/${encodeURIComponent(leg.account_ref)}/transactions?${params.toString()}`,
      )
    ...
    <div style={{ marginTop: "8px" }}>
      ...
      {open && (
        <div style={{ marginTop: "8px", paddingLeft: "12px" }}>
          ...
          {txns?.length === 0 && (
            <div className="card-sub">No transactions in billing period.</div>
          )}
          {txns?.map((t) => (
            <div
              key={t.idempotency_hash}
  ...
  return (
    <div className="screen">
      ...
      {pairings.map((pairing) => {
        ...
        const rawCcLeg =
          pairing.event_type === MARKED_CC_PAYMENT
            ? pairing.legs.find((l) => l.role === "cc_credit")
            : null;
        const ccLeg =
          rawCcLeg?.account_ref && rawCcLeg.period_start && rawCcLeg.period_end
            ? rawCcLeg
            : null;
          ...
          <div key={pairing.event_id} className="card">
            ...
            >
              <Badge variant="default">
                {pairingLabel(pairing.event_type)}
              </Badge>
              <span className="card-sub" style={{ margin: 0 }}>
            ...
            <div className="card-sub" style={{ marginBottom: 0 }}>
              {pairing.legs.map((leg) => (
                <div key={leg.idempotency_hash}>
                  {leg.role}: {leg.account_ref} ({leg.period_start ?? "?"} →{" "}
                  {leg.period_end ?? "?"})
                </div>
              ))}
            </div>
