...
describe('A1: sync history renders real fetched data', () => {
  it('renders account_ref from API response', async () => {
    ...
    await waitFor(() => expect(screen.getByText('HDFC_SAVINGS')).toBeInTheDocument());
  ...
  it('marks a stalled entry with a warning badge', async () => {
    ...
    render(<SyncHistoryPage />);
    await waitFor(() => expect(screen.getByText('SBI_SAVINGS')).toBeInTheDocument());
...
describe('A2: overlap map renders real fetched data', () => {
  it('renders account_ref and statement period', async () => {
    ...
    await waitFor(() => expect(screen.getByText('HDFC_CC')).toBeInTheDocument());
...
describe('A3: dedup ledger renders real fetched data', () => {
  ...
  it('renders excluded row with warning badge and ingestion event back-reference', async () => {
    ...
    render(<DedupLedgerPage />);
    await waitFor(() => expect(screen.getByText('CC payment excluded')).toBeInTheDocument());
    ...
    mockFetch('resolver-pairings', [
      {
        ...
        legs: [
          { role: 'debit', idempotency_hash: 'hash1', account_ref: 'HDFC_SAVINGS', period_start: '2026-02-01', period_end: '2026-02-28' },
          { role: 'credit', idempotency_hash: 'hash2', account_ref: 'SBI_SAVINGS', period_start: '2026-02-01', period_end: '2026-02-28' },
        ],
      },
    ]);

    render(<PairingsPage />);
    await waitFor(() => expect(screen.getByText('Internal Transfer')).toBeInTheDocument());
    ...
    mockFetch('resolver-pairings', [
      {
        ...
        legs: [
          { role: 'savings_debit', idempotency_hash: 'hashS', account_ref: 'HDFC_SAVINGS', period_start: '2026-03-01', period_end: '2026-03-31' },
          // cc_credit leg has empty account_ref — join missed (CC not yet ingested)
          { role: 'cc_credit', idempotency_hash: 'hashCC', account_ref: '', period_start: null, period_end: null },
        ],
      },
    ]);
...
describe('A6: CC payment expand fetches account transactions for billing period', () => {
  it('shows CC purchases on expand using period from the cc_credit leg', async () => {
      ...
      'resolver-pairings': [
        {
          ...
          legs: [
            { role: 'savings_debit', idempotency_hash: 'hashS', account_ref: 'HDFC_SAVINGS', period_start: '2026-03-01', period_end: '2026-03-31' },
            { role: 'cc_credit', idempotency_hash: 'hashCC', account_ref: 'HDFC_CC_9876', period_start: '2026-02-15', period_end: '2026-03-14' },
          ],
        },
      ],
    ...
    await waitFor(() => expect(screen.getByText('SWIGGY ORDER')).toBeInTheDocument());
